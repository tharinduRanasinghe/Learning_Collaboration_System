...
//$_SESSION['student_id']='s01@gmail.com';
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>AUAHA - Student dashboard</title>
        ...
        <div class="container-fluid">
            ...
            <div class="row">
               ...
               echo "</div>";
                                            ...
                                            $query = "SELECT count(*) as count1 FROM threads t left outer join users u on t.thread_user_id=u.sno where u.user_email in ( select tid from course c inner join studentcourse sc on c.CourseId=sc.CourseId where sc.Sid='$sid')";

                                            $result_set = mysqli_query($connection, $query);
                                            if ($result_set) {

                                                    while ($record = mysqli_fetch_assoc($result_set)) {
                                                     echo "<div class = 'mr-5'><h5>" .$record['count1']." Forum Questions!</h5></div>";
            ...
            ?>
                 ...
                 $sid = $_SESSION['student_id'];
                             ...
                             $query = "SELECT count(*) as count1 FROM studentcourse where Sid='$sid'";
              ...
              echo "<div class='mr-5'><h5>".$record['count1']." Courses!</h5></div>";
                                ...
                                <a class="card-footer text-white clearfix small z-1" href="grades.php">
                                    <span class="float-left">View Details</span>
                                    <span class="float-right">
                                        <i class="fa fa-angle-right"></i>
                                    </span>
                 ...
                 $sid = $_SESSION['student_id'];
                             ...
                             $query = "SELECT count(*) as count1 FROM quiz q inner join studentcourse sc on q.cid=sc.CourseId where sc.Sid='$sid' and q.eid not in (select eid from history where sid='$sid')";

                             $result_set = mysqli_query($connection, $query);
                             if ($result_set) {

                                     while ($record = mysqli_fetch_assoc($result_set)) {
              echo "<div class='mr-5'><h5>".$record['count1']." Exams left!</h5></div>";
                                     }
                             }
                      ?>
                                </div>
                                <a class="card-footer text-white clearfix small z-1" href="attemptQuiz.php?q=5">
                                    <span class="float-left">View Details</span>
                                    <span class="float-right">
                                        <i class="fa fa-angle-right"></i>
                                    </span>
               ...
               $sid = $_SESSION['student_id'];
                           ...
                           $query = "SELECT count(*) as count1 FROM rank where Sid='$sid'";

                           $result_set = mysqli_query($connection, $query);
                           if ($result_set) {

                                   while ($record = mysqli_fetch_assoc($result_set)) {
              echo "<div class='mr-5'><h5>".$record['count1']." Results released!</h5></div>";
                                   }
                           }
                    ?>
                                </div>
                                <a class="card-footer text-white clearfix small z-1" href="attemptQuiz.php?q=4">
                                    <span class="float-left">View Details</span>
                                    <span class="float-right">
                                        <i class="fa fa-angle-right"></i>
                                    </span>
          ...
          $labels=[];
          $data = [];
          $result = mysqli_query($connection,"SELECT c.CourseName,AVG(r.score/r.fullMarks) as Average from rank r inner join course c on r.cid=c.CourseId where r.Sid='$sid' GROUP BY c.CourseName") or die('Error');
          while($row = mysqli_fetch_array($result)) {
            $labels[] = $row['CourseName'];
